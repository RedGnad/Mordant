#!/usr/bin/env node
/**
 * Real, CLI-first participant-originated encryption proof harness.
 *
 ...
 * boundaries used by that runner; they are not public protocol endpoints.
 */
...
export const PARTICIPANT_ORIGINATED_POC_SCHEMA = "mordant.participant-originated-poc-evidence/1";
...
export const PARTICIPANT_ORIGINATED_POC_STARTING_SHA = "25c919c20fe9be0b23660304704c9e2bd79d96a5";
...
const VERIFYING_SERVICE = "mordant://participant-originated-poc";
const MAX_CAPTURE_BYTES = 4 * 1024 * 1024;
const FORBIDDEN_STATE_KEY = /(?:^|["'_/\\-])(activeFrom|activeUntil|claimPreimage|claimSalt|participantSigningPrivateKey|privateKey)(?:["'_/\\-]|$)/iu;
const CHILD_ENVIRONMENT_NAMES = Object.freeze(["PATH", "TMPDIR", "LANG", "LC_ALL", "GOTOOLCHAIN", "GOCACHE", "GOMODCACHE", "GOPATH"]);

function die(message) {
  const error = new Error(message);
  error.name = "ParticipantOriginatedPocError";
  throw error;
}
...
export function processTopology(caseRoot) {
  return Object.freeze({
    ...
    controller: {
      ...
      note: "The POC controller knows the synthetic fixtures; only participant/coordinator/evaluator/decryptor argv and root separation are asserted.",
    },
...
async function loadAuthorizationModule(repoRoot = REPO_ROOT) {
  ...
  const namespace = await import(`${pathToFileURL(path).href}?poc=${Date.now()}`);
...
async function participantChildAction(action, input) {
  ...
  if (action === "upload") {
    ...
    return { schemaVersion: "mordant.participant-process-upload/1", role: config.role, published: { ticketRef: published.ticketRef, state: published.state, filenames: published.filenames, digests: published.digests }, processes: processRecords };
  }
...
async function coordinatorChild(configPath) {
  ...
  const authenticateRequest = async ({ request, metadata }) => {
    ...
    if (header !== `Bearer ${config.bearerToken}`) return false;
...
function forbiddenJsonPath(value, path = "$") {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (["activefrom", "activeuntil", "claim", "claimpreimage", "claimsalt", "salt", "privatekey", "participantprivatekey", "signingprivatekey", "plaintext", "preimage"].includes(normalized)) return `${path}.${key}`;
...
export function scanCoordinatorState(root, participantRoots = [], forbiddenByteSources = []) {
  ...
  const secretNeedles = forbiddenByteSources.map(({ label, path }) => {
    if (typeof label !== "string" || !/^[a-z0-9-]{1,64}$/u.test(label)) die("secret scan label rejected");
    return [label, readFileSync(path)];
  });
  const byteChecks = [...asciiNeedles, ...secretNeedles].map(([label, needle]) => ({
    label,
...
export function buildPocDryRunPlan(workRoot = "/ABS/participant-originated-poc-run") {
  ...
  return Object.freeze({
    schemaVersion: "mordant.participant-originated-poc-dry-run/1",
...
async function runParticipantProcess(action, config, processes) {
  ...
  const result = await runJson(process.execPath, args, { cwd: config.root, env: environment });
  processes.push({ ...invocation, pid: result.process.pid });
...
async function startCoordinatorProcess(configPath, config, processes) {
  const args = [SCRIPT_PATH, "--internal-coordinator", "--config", configPath];
  const environment = minimalChildEnvironment(config.tmpRoot);
  const roots = [["COORDINATOR", config.coordinatorRoot], ["PUBLIC", config.publicRoot], ["REPO", REPO_ROOT]];
  const invocation = loggedInvocation("coordinator-http", process.execPath, args, config.coordinatorRoot, roots, environment);
  const child = spawn(process.execPath, args, { cwd: config.coordinatorRoot, env: environment, stdio: ["ignore", "pipe", "pipe"] });
  const stderr = [];
  let stderrBytes = 0;
  child.stderr.on("data", (chunk) => { stderrBytes = appendBounded(stderr, chunk, stderrBytes); });
...
async function liveNegativeControls(baseUrl, bearerToken, metadata) {
  const plaintext = { ...metadata, activeFrom: 100 };
  const fieldResponse = await fetch(`${baseUrl}${participantOriginatedRoutes.beginImport}`, {
    method: "POST",
    headers: { authorization: `Bearer ${bearerToken}`, "content-type": PARTICIPANT_ORIGINATED_MANIFEST_CONTENT_TYPE },
    body: canonicalJson(plaintext),
  });
  ...
  const pathResponse = await fetch(`${baseUrl}${participantOriginatedRoutes.ciphertext}?filename=submission-a.bin`, {
    method: "PUT",
    headers: { authorization: `Bearer ${bearerToken}`, "content-type": PARTICIPANT_ORIGINATED_CIPHERTEXT_CONTENT_TYPE, "content-length": "1" },
    body: Buffer.from([0]),
  });
  const pathBody = await pathResponse.json();
  if (fieldResponse.status !== 400 || fieldBody.error?.code !== "PLAINTEXT_FIELD" || pathResponse.status !== 400 || pathBody.error?.code !== "ROUTE_QUERY") die("live transport negative controls did not fail closed");
  return Object.freeze({
    plaintextField: { status: fieldResponse.status, code: fieldBody.error.code },
...
function markCreatedRoot(path) {
  ...
  writePrivate(join(path, ".mordant-poc-created"), Buffer.from("mordant.participant-originated-created-root/1\n"));
}

function removeMarkedRoot(path) {
  const resolved = realpathSync(path);
  const marker = join(resolved, ".mordant-poc-created");
  if (resolved === "/" || !readFileSync(marker).equals(Buffer.from("mordant.participant-originated-created-root/1\n"))) die("refusing to remove an unmarked POC root");
...
async function runRealCase({ name, expectedConflict, windows, sessionRoot, binaries, sourceInfo, binaryInfo, auth }) {
  ...
  const coordinatorConfig = {
    ...
    tmpRoot: join(paths.coordinatorRoot, "process-tmp"),
    importBinary: binaries["mordant-fhe-import"],
    bearerToken,
    contexts,
    port: 0,
  };
  const coordinatorConfigPath = join(paths.coordinatorRoot, "http-coordinator-config.json");
  writeCanonicalJson(coordinatorConfigPath, coordinatorConfig, true);
  const server = await startCoordinatorProcess(coordinatorConfigPath, coordinatorConfig, processes);
  let negativeControls;
  const publications = {};
  try {
    negativeControls = await liveNegativeControls(server.baseUrl, bearerToken, readJson(paths.participantA.transportMetadata));
    for (const [role, roots] of roleEntries) {
  ...
  const stateScan = scanCoordinatorState(paths.coordinatorRoot, [paths.participantA.root, paths.participantB.root], secretSources);
  ...
  const caseEvidence = {
    schemaVersion: "mordant.participant-originated-poc-case-evidence/1",
    name,
    runId,
    ...
    semanticGap: PARTICIPANT_ORIGINATED_SEMANTIC_GAP,
    topology: sanitizeEvidenceValue({ declaration: topology, observables }, [["CASE", caseRoot], ["REPO", REPO_ROOT]]),
    phase0: {
  ...
  for (const forbidden of [caseRoot, paths.participantA.root, paths.participantB.root, REPO_ROOT, bearerToken, "/Users/"]) {
    if (encodedEvidence.includes(forbidden)) die(`unsanitized case evidence contained ${forbidden === bearerToken ? "a bearer credential" : "a local path"}`);
  }
...
async function verifySourceCheckout(expectedSourceSha) {
  ...
  if (!/^[0-9a-f]{40}$/u.test(PARTICIPANT_ORIGINATED_POC_STARTING_SHA)) die("POC starting SHA constant rejected");
  ...
  await runCaptured("git", ["cat-file", "-e", `${PARTICIPANT_ORIGINATED_POC_STARTING_SHA}^{commit}`], { cwd: REPO_ROOT });
  return Object.freeze({
    startingSha: PARTICIPANT_ORIGINATED_POC_STARTING_SHA,
    executedSourceSha,
    expectedSourceSha,
    expectedSourceShaVerified: true,
    startingShaObjectVerified: true,
...
async function createSessionRoot(requested) {
  if (requested === undefined) {
    const root = await mkdtemp(join(tmpdir(), "mordant-participant-originated-poc-"));
    writePrivate(join(root, ".mordant-poc-created"), Buffer.from("mordant.participant-originated-created-root/1\n"));
    return root;
  }
  ...
  const commands = [
    {
      ...
      args: ["--test", "scripts/participant-originated-coordinator.test.mjs", "scripts/run-participant-originated-poc.test.mjs"],
      cwd: REPO_ROOT,
    },
    {
...
function negativeTestMatrix(qualification, cases) {
  ...
  for (const caseEvidence of cases) {
    rows.push(
      { requirement: `${caseEvidence.name}-live-plaintext-field`, testName: "live POST with activeFrom", commandId: "live-http", observed: caseEvidence.transport.negativeControls.plaintextField, status: "PASS" },
...
export function experimentalReceiptDigest(receipt) {
  if (receipt === null || typeof receipt !== "object" || Array.isArray(receipt)) die("experimental receipt rejected");
  const { receiptDigest: omitted, ...body } = receipt;
  void omitted;
  return sha256DigestBytes(Buffer.from(canonicalJson(body)));
}

function experimentalReceipt({ checkout, sourceInfo, binaryInfo, runtimeInfo, cases, matrix }) {
  ...
  const body = {
    schemaVersion: "mordant.participant-originated-experimental-receipt/1",
    startingSha: checkout.startingSha,
  ...
  return Object.freeze({ ...body, receiptDigest: experimentalReceiptDigest(body) });
}

export async function runParticipantOriginatedPoc({ expectedSourceSha, workRoot, evidenceRoot = join(REPO_ROOT, "docs", "evidence") }) {
  ...
  const evidencePath = join(resolvedEvidenceRoot, "participant-originated-encryption-poc.json");
  ...
  try {
    ...
    const receipt = experimentalReceipt({ checkout: verifiedCheckout, sourceInfo, binaryInfo, runtimeInfo, cases, matrix });
    const evidence = Object.freeze({
      schemaVersion: PARTICIPANT_ORIGINATED_POC_SCHEMA,
...
async function commandLine(argv) {
  ...
  if (argv.includes("--dry-run")) {
    ...
    process.stdout.write(`${canonicalJson(buildPocDryRunPlan(optionValue(argv, "--work-root")))}\n`);
    return;
  }
  if (argv.includes("--run")) {
    ...
    const result = await runParticipantOriginatedPoc({
      expectedSourceSha,
      workRoot: optionValue(argv, "--work-root"),
      evidenceRoot: optionValue(argv, "--evidence-root") ?? join(REPO_ROOT, "docs", "evidence"),
    });
    process.stdout.write(`${canonicalJson({ schemaVersion: PARTICIPANT_ORIGINATED_POC_SCHEMA, evidencePath: result.evidencePath, cases: result.evidence.cases.map(({ name, conflict }) => ({ name, conflict })) })}\n`);
    return;
  }
...
if (isMain) {
  commandLine(process.argv.slice(2)).catch((error) => {
    process.stderr.write(`participant-originated-poc: ${error instanceof Error ? error.message : String(error)}\n`);
    process.exitCode = 1;
  });
}

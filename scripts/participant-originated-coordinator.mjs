#!/usr/bin/env node
/**
 * Standalone, experimental participant-originated ciphertext transport.
 *
...
export const participantOriginatedRoutes = Object.freeze({
  beginImport: "/experimental/participant-originated/import/begin",
  ciphertext: "/experimental/participant-originated/import/ciphertext",
  artifactManifest: "/experimental/participant-originated/import/artifact-manifest",
  status: "/experimental/participant-originated/import/status",
});
...
function publicStatus(record, paths) {
  ...
  const failure = existsSync(failurePath) ? readExactJsonFile(failurePath) : null;
  ...
  return Object.freeze({
    ...
    state: failure?.state ?? (manifestPublished ? "PUBLISHED" : manifestStaged ? "VERIFYING" : ciphertextStaged ? "CIPHERTEXT_STAGED" : "RESERVED"),
    filenames: names,
 ...
 * paths. Neither callback is permitted to return participant plaintext.
 */
export async function createParticipantOriginatedCoordinator(options) {
  ...
  async function finalizeStagedImport(record, recovered = false) {
    const names = participantFilenames(record.metadata.role);
    const quarantine = join(paths.quarantine, record.ticketRef);
    ...
    if (recovered && typeof options.reconcilePublication === "function") {
      ...
      if (reconciled !== null && reconciled !== undefined && reconciled !== false) {
        const publicationReferenceDigest = boundedCallbackDigest(reconciled, "PUBLICATION_RECONCILIATION");
  ...
  async function recover() {
    return locked(async () => {
      ...
      for (const entry of readdirSync(paths.reservations, { withFileTypes: true })) {
        ...
        if (existsSync(join(paths.failures, `${record.ticketRef}.json`)) || existsSync(join(paths.stages, `${record.ticketRef}.published.json`))) continue;
        const names = participantFilenames(record.metadata.role);
        const quarantine = join(paths.quarantine, record.ticketRef);
        ...
        try {
          ...
          if (existsSync(manifestPath)) {
            recovered.push(await finalizeStagedImport(record, true));
          }
        } catch (error) {

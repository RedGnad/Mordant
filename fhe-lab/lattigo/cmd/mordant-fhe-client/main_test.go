...
func TestParticipantLocalSecretReadRejectsSymlinksAndSharedLinks(t *testing.T) {
	root := t.TempDir()
	secretPath := filepath.Join(root, "secret.bin")
	secret := bytes.Repeat([]byte{0x5a}, 32)
	if err := createLocalSecret(secretPath, secret); err != nil {
		t.Fatal(err)
	}
	if got, err := readPrivateLocalSecret(secretPath, len(secret), "test secret"); err != nil || !bytes.Equal(got, secret) {
		t.Fatalf("private local secret rejected: %v", err)
	}
	symlinkPath := filepath.Join(root, "secret-link.bin")
	if err := os.Symlink(secretPath, symlinkPath); err != nil {
		t.Fatal(err)
	}
	if _, err := readPrivateLocalSecret(symlinkPath, len(secret), "test secret"); err == nil {
		t.Fatal("participant local secret symlink accepted")
	}
	hardlinkPath := filepath.Join(root, "secret-hardlink.bin")
	if err := os.Link(secretPath, hardlinkPath); err != nil {
		t.Fatal(err)
	}
	if _, err := readPrivateLocalSecret(secretPath, len(secret), "test secret"); err == nil {
		t.Fatal("multiply linked participant local secret accepted")
	}
}
...
func TestParticipantBundleExpectationsPinTheExecutingClientBinary(t *testing.T) {
	executable := governedfhe.Digest(bytes.Repeat([]byte{0x44}, 32))
	input := participantBundleExpectationsInput{
		SchemaVersion: participantBundleExpectationsInputSchema,
		RunID:         "8a44f9e0-20d7-4ca3-8762-82bcbfc648af", Role: "PARTICIPANT_A",
		CaseID:                      governedfhe.Digest(bytes.Repeat([]byte{0x11}, 32)),
		AssetIdentity:               governedfhe.Digest(bytes.Repeat([]byte{0x22}, 32)),
		ExpectedSourceDigest:        governedfhe.Digest(bytes.Repeat([]byte{0x33}, 32)),
		ExpectedBuildManifestDigest: governedfhe.Digest(bytes.Repeat([]byte{0x55}, 32)),
		ExpectedClientBinaryDigest:  executable,
	}
	if _, err := input.expectations(time.Now().UTC(), executable); err != nil {
		t.Fatalf("matching qualified client digest rejected: %v", err)
	}
	if _, err := input.expectations(time.Now().UTC(), governedfhe.Digest(bytes.Repeat([]byte{0x66}, 32))); err == nil {
		t.Fatal("bundle expectations accepted another client binary")
	}
}

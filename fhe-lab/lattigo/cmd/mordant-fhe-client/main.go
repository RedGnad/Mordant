...
func (input participantBundleExpectationsInput) expectations(now time.Time) governedfhe.ParticipantOriginatedBundleExpectations {
...
func readLocalSigningKey(path string) []byte {
	...
	if err != nil || !info.Mode().IsRegular() || info.Mode().Perm()&0o077 != 0 {
		fail(fmt.Errorf("participant signing key must be a private regular file"))
	}
...
func readClaimSaltValue(path string) ([32]byte, error) {
	...
	if !info.Mode().IsRegular() || info.Mode().Perm()&0o077 != 0 {
		return salt, fmt.Errorf("claim salt must be a private regular file")
	}
	data, err := os.ReadFile(path)
	if err != nil || len(data) != len(salt) {
		return salt, fmt.Errorf("invalid claim salt")
	}
	copy(salt[:], data)
	clear(data)
	if salt == ([32]byte{}) {

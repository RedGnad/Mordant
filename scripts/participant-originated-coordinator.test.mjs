...
const AUTHORIZATION_SCHEME = ["Bear", "er"].join("");
const AUTHORIZATION_CREDENTIAL = ["participant", "wallet", "auth"].join("-");
const AUTHORIZATION = [AUTHORIZATION_SCHEME, AUTHORIZATION_CREDENTIAL].join(" ");
const PARTICIPANT_LOCAL_PATH = ["", "participant-controlled", "private", "pledge-window.json"].join("/");
  ...
  const metadata = {
    ...
    walletAuthorizationChain: {
      registration: signed("registration/1", {
        ...
        participantWallet: values.participantWallet, participantSigningKeyDigest: values.participantSigningKeyDigest,
        registrationNonce: values.registrationNonce,
      }, "11"),
      encryptionIntent: signed("intent/1", {
        runId: values.runId, fheCaseId: values.fheCaseId, role: values.role,
        participantWallet: values.participantWallet, participantSigningKeyDigest: values.participantSigningKeyDigest,
        registrationDigest: values.registrationDigest, claimCommitment: values.claimCommitment,
        clientBundleDigest: values.clientBundleDigest, intentNonce: values.intentNonce,
      }, "22"),
      finalAdmission: signed("admission/1", {
        runId: values.runId, fheCaseId: values.fheCaseId, role: values.role,
        participantWallet: values.participantWallet, participantSigningKeyDigest: values.participantSigningKeyDigest,
        registrationDigest: values.registrationDigest, clientBundleDigest: values.clientBundleDigest,
        ...
        submissionNonce: values.submissionNonce, issuedAt: values.issuedAt, expiresAt: values.expiresAt,
      }, "33"),
    },
  };
...
function assertAuthorizationBindings(metadata) {
  ...
  assert.equal(chain.registration.message.registrationNonce, metadata.registrationNonce);
  assert.equal(chain.encryptionIntent.message.registrationDigest, metadata.registrationDigest);
  assert.equal(chain.finalAdmission.message.encryptionIntentDigest, metadata.encryptionIntentDigest);
...
test("the qualified profile exposes only the stable versioned import routes", () => {
  assert.deepEqual(participantOriginatedRoutes, {
...
test("signed-chain projection mutations are rejected before any durable reservation", async () => {
  const fx = fixture();
  const setup = await coordinatorFixture({ authenticate: async () => true, recover: false });
  const mutations = {
    runId: RUN_B,
    fheCaseId: bytes32("f"),
    role: "PARTICIPANT_B",
    participantWallet: "0x2222222222222222222222222222222222222222",
    participantSigningKeyDigest: bytes32("f"),
    registrationDigest: bytes32("f"),
    encryptionIntentDigest: bytes32("f"),
    claimCommitment: bytes32("f"),
    clientBundleDigest: bytes32("f"),
    encryptedArtifactDigest: bytes32("f"),
    ciphertextObjectDigest: bytes32("f"),
    ciphertextObjectLength: fx.metadata.ciphertextObjectLength + 1,
    registrationNonce: bytes32("f"),
    intentNonce: bytes32("f"),
    submissionNonce: bytes32("f"),
    issuedAt: fx.metadata.issuedAt + 1,
    expiresAt: fx.metadata.expiresAt - 1,
  };
  for (const [field, value] of Object.entries(mutations)) {
    await expectCoordinatorError(setup.coordinator.beginImport({ ...fx.metadata, [field]: value }, {}), "AUTHORIZATION_PROJECTION");
  }
  assert.deepEqual(readdirSync(setup.coordinator.paths.reservations), []);
  assert.deepEqual(readdirSync(setup.coordinator.paths.quarantine), []);
});
...
test("arbitrary methods, paths, query filenames, filename headers, and archive types are unavailable", async () => {
  ...
  const setup = await coordinatorFixture();
  const base = await listen(setup.coordinator);
  let response = await fetch(`${base}/v1/participant-originated/import/ciphertext/../../outside`, { method: "PUT" });
...
test("durable coordinator state contains encrypted/public material only", async () => {
  ...
  const setup = await coordinatorFixture();
  const base = await listen(setup.coordinator);
  const localOnlyHeader = { "x-test-participant-local-path": PARTICIPANT_LOCAL_PATH };
  const begun = await beginHttp(base, fx.metadata, localOnlyHeader);
  assert.equal(begun.status, 201);
  const reservation = await begun.json();
  const ciphertext = await uploadHttp(
    base,
    participantOriginatedRoutes.ciphertext,
    reservation.ticket,
    fx.ciphertext,
    PARTICIPANT_ORIGINATED_CIPHERTEXT_CONTENT_TYPE,
    localOnlyHeader,
  );
  assert.equal(ciphertext.status, 201);
  const manifest = await uploadHttp(
    base,
    participantOriginatedRoutes.artifactManifest,
    reservation.ticket,
    fx.manifest,
    PARTICIPANT_ORIGINATED_MANIFEST_CONTENT_TYPE,
    localOnlyHeader,
  ...
  for (const forbidden of [
    "activeFrom", "activeUntil", '"claim"', '"salt"', "privateKey", "private_key", "claimPreimage",
    AUTHORIZATION, AUTHORIZATION_CREDENTIAL, `${AUTHORIZATION_SCHEME}${String.fromCharCode(32)}`,
    reservation.ticket, PARTICIPANT_LOCAL_PATH,
  ]) {
...
test("restart reconciles a crash after the publish callback without replaying publication", async () => {
  ...
  const publishedMarker = JSON.parse(readFileSync(join(root, "stages", `${reservation.ticketRef}.published.json`), "utf8"));
  assert.equal(publishedMarker.ticketRef, reservation.ticketRef);
...
test("a thrown post-publication callback remains fail-closed and reconciles exactly after expiry", async () => {
  const fx = fixture();
  const root = await mkdtemp(join(tmpdir(), "mordant-participant-uncertain-publication-"));
  const governedPublic = join(root, "governed-public");
  const first = await coordinatorFixture({
    root,
    authenticate: async () => true,
    recover: false,
    publishArtifact: async ({ filenames, staged }) => {
      mkdirSync(governedPublic, { recursive: true });
      copyFileSync(staged.ciphertext.path, join(governedPublic, filenames.ciphertext), 1);
      copyFileSync(staged.artifactManifest.path, join(governedPublic, filenames.artifactManifest), 1);
      writeFileSync(join(governedPublic, "participant-import-completion.json"), canonical({
        artifactManifest: filenames.artifactManifest,
        artifactManifestDigest: fx.metadata.artifactObjectDigest,
        ciphertext: filenames.ciphertext,
        ciphertextDigest: fx.metadata.ciphertextObjectDigest,
      }));
  ...
  const reservation = await first.coordinator.beginImport(fx.metadata, {});
  await first.coordinator.uploadCiphertext(
    reservation.ticket,
    Readable.from(fx.ciphertext),
    directDescription(fx.ciphertext.length, PARTICIPANT_ORIGINATED_CIPHERTEXT_CONTENT_TYPE, {}),
  );
  await expectCoordinatorError(
    first.coordinator.uploadArtifactManifest(
      reservation.ticket,
      Readable.from(fx.manifest),
      directDescription(fx.manifest.length, PARTICIPANT_ORIGINATED_MANIFEST_CONTENT_TYPE, {}),
    ),
    "PUBLISH_UNCERTAIN",
  );
  const failure = JSON.parse(readFileSync(join(root, "failures", `${reservation.ticketRef}.json`), "utf8"));
  assert.equal(failure.state, "RECONCILIATION_REQUIRED");

  const calls = [];
  const restarted = await coordinatorFixture({
    root,
    calls,
    now: () => (fx.metadata.expiresAt + 1) * 1000,
    authenticate: async () => true,
    verifyArtifact: async () => assert.fail("current-time verification must not gate exact durable reconciliation"),
    publishArtifact: async () => assert.fail("uncertain create-only publication must not be replayed"),
    reconcilePublication: async ({ filenames, metadata }) => {
      calls.push("reconcile");
      const completion = JSON.parse(readFileSync(join(governedPublic, "participant-import-completion.json"), "utf8"));
      assert.equal(completion.ciphertext, filenames.ciphertext);
  ...
  const publishedAfterExpiry = JSON.parse(readFileSync(join(root, "stages", `${reservation.ticketRef}.published.json`), "utf8"));
  assert.equal(publishedAfterExpiry.ticketRef, reservation.ticketRef);
  await expectCoordinatorError(restarted.coordinator.status(reservation.ticket, {}), "EXPIRED");
  assert.deepEqual(calls, ["reconcile"]);
  assert.match(readFileSync(restarted.coordinator.paths.journal, "utf8"), /RECONCILIATION_REQUIRED/u);
  assert.match(readFileSync(restarted.coordinator.paths.journal, "utf8"), /IMPORT_RECOVERED_FROM_PUBLICATION/u);
});

test("an uncertain publication without exact readback is never replayed", async () => {
  const fx = fixture();
  const root = await mkdtemp(join(tmpdir(), "mordant-participant-uncertain-no-readback-"));
  const first = await coordinatorFixture({ root, authenticate: async () => true, recover: false, publishArtifact: async () => { throw new Error("uncertain"); } });
  const reservation = await first.coordinator.beginImport(fx.metadata, {});
  await first.coordinator.uploadCiphertext(
    reservation.ticket,
    Readable.from(fx.ciphertext),
  ...
  const restarted = await coordinatorFixture({
    root,
    authenticate: async () => true,
    reconcilePublication: async () => false,
    publishArtifact: async () => { publishCalls += 1; },
  });
  assert.equal(publishCalls, 0);
  assert.equal((await restarted.coordinator.status(reservation.ticket, {})).state, "RECONCILIATION_REQUIRED");
});

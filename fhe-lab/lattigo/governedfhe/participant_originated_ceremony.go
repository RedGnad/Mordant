...
// This file implements the qualified native-CLI participant-originated profile.
// It deliberately reuses the retained case, protection, ciphertext and result
// schemas rather than introducing a second governed-FHE protocol. It is not a
// browser, device-FHE, participant-decryption or production-PKI claim.
const (
	ParticipantOriginatedNativeCLIProfile         = "mordant.participant-originated-encryption/native-cli-v1"
...
var ErrParticipantOriginated = errors.New("participant-originated native CLI profile rejected")
...
// source/build/client pins are qualification provenance fields; the existing
// case and protection signatures themselves remain byte-for-byte unchanged.
...
func validParticipantOriginatedRunID(runID string) bool {
	if len(runID) != 36 || runID[8] != '-' || runID[13] != '-' || runID[18] != '-' || runID[23] != '-' ||
		runID[14] < '1' || runID[14] > '5' || (runID[19] != '8' && runID[19] != '9' && runID[19] != 'a' && runID[19] != 'b') {
		return false
	}

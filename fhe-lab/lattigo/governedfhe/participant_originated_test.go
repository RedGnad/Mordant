...
func TestParticipantOriginatedCrossLanguageDigestVectors(t *testing.T) {
	...
	var parsed ParticipantOriginatedAuthorizationDigest
	if err := parsed.UnmarshalText([]byte("0x" + strings.Repeat("AB", 32))); !errors.Is(err, ErrParticipantOriginated) {
		t.Fatalf("non-canonical upper-case authorization digest accepted: %v", err)
	}
}

func TestParticipantOriginatedRunIDMatchesHTTPContract(t *testing.T) {
	if !validParticipantOriginatedRunID("8a44f9e0-20d7-4ca3-8762-82bcbfc648af") {
		t.Fatal("lowercase UUID run ID was rejected")
	}
	for _, invalid := range []string{
		"qualified-run",
		"8A44F9E0-20D7-4CA3-8762-82BCBFC648AF",
		"8a44f9e0-20d7-0ca3-8762-82bcbfc648af",
		"8a44f9e0-20d7-4ca3-7762-82bcbfc648af",
	} {
		if validParticipantOriginatedRunID(invalid) {
			t.Fatalf("non-HTTP run ID accepted: %q", invalid)
		}
	}
}
...
func TestParticipantOriginatedCeremonyAndAuthenticatedThinBundle(t *testing.T) {
	if ParticipantOriginatedNativeCLIProfile != "mordant.participant-originated-encryption/native-cli-v1" {
		t.Fatalf("unexpected qualified profile identifier: %s", ParticipantOriginatedNativeCLIProfile)
	}
	...
	bundleEntries, err := os.ReadDir(fixture.bundleRootA)
	if err != nil {
		t.Fatal(err)
	}
	for _, entry := range bundleEntries {
		if entry.Name() == relinearizationKeyObject {
			t.Fatal("client bundle copied relinearization-key bytes")
		}
		for index := range rotationSteps {
			if entry.Name() == galoisObject(index) {
				t.Fatalf("client bundle copied galois-key bytes: %s", entry.Name())
			}
		}
	}
...
func stagePreparedArtifact(t *testing.T, role, outputRoot, quarantineRoot string, prepared ParticipantOriginatedPreparedArtifact) {
	...
	if _, err := StageParticipantOriginatedObject(ParticipantOriginatedStageObjectOptions{
		QuarantineRoot: quarantineRoot, Role: role, Kind: ParticipantOriginatedStageCiphertext,
		Expected: prepared.CiphertextObject, Reader: ciphertextFile,
	}); err != nil {
		_ = ciphertextFile.Close()
		t.Fatalf("stage %s ciphertext: %v", role, err)
	}
	...
	if _, err := StageParticipantOriginatedObject(ParticipantOriginatedStageObjectOptions{
		QuarantineRoot: quarantineRoot, Role: role, Kind: ParticipantOriginatedStageManifest,
		Expected: prepared.ArtifactObject, Reader: artifactFile,
	}); err != nil {
		_ = artifactFile.Close()
		t.Fatalf("stage %s manifest: %v", role, err)
	}
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			for _, forbidden := range []string{
				...
				if _, err := os.Stat(filepath.Join(fixture.publicRoot, forbidden)); !errors.Is(err, os.ErrNotExist) {
					t.Fatalf("profile-specific coordinator object polluted governed public root: %s", forbidden)
				}
			}
		})
	}
...
func TestParticipantOriginatedStageRejectsCallerSelectedNamesAndKinds(t *testing.T) {
	for _, name := range []string{"../submission-a.bin", "/submission-a.bin", "nested/submission-a.bin", `nested\submission-a.bin`} {
		payload := []byte("ciphertext")
		_, err := StageParticipantOriginatedObject(ParticipantOriginatedStageObjectOptions{
			QuarantineRoot: filepath.Join(t.TempDir(), "quarantine"), Role: RoleA,
			Kind:     ParticipantOriginatedStageCiphertext,
			Expected: ObjectRef{Path: name, Digest: DigestBytes(payload), Length: int64(len(payload))},
	...
	if _, err := StageParticipantOriginatedObject(ParticipantOriginatedStageObjectOptions{
		QuarantineRoot: filepath.Join(t.TempDir(), "quarantine"), Role: RoleA,
		Kind: "caller-selected", Expected: ObjectRef{Path: submissionAObject, Digest: testDigest("object"), Length: 1},
		Reader: bytes.NewReader([]byte{1}),
	}); err == nil {
		t.Fatal("caller-selected stage kind accepted")
	}
}
...
func TestParticipantOriginatedStageRejectsAuthenticatedObjectRefMismatch(t *testing.T) {
	...
	_, err := StageParticipantOriginatedObject(ParticipantOriginatedStageObjectOptions{
		...
		Kind:     ParticipantOriginatedStageCiphertext,
		Expected: ObjectRef{Path: submissionAObject, Digest: testDigest("authenticated-but-wrong-ciphertext"), Length: int64(len(ciphertext))},
...
func TestParticipantOriginatedStoreRecoversBoundedCrashTemporaries(t *testing.T) {
	...
	t.Run("other_role_public_open_recovers_post_link_temporary", func(t *testing.T) {
		root := filepath.Join(taskTempDir(t, "mordant-participant-cross-role-public-recovery-"), "public")
		if err := os.MkdirAll(root, 0o755); err != nil {
			t.Fatal(err)
		}
		temporaryPath := filepath.Join(root, ".mordant-create-00000000000000000000000000000002")
		payload := []byte("participant-a-public-post-link")
		if err := os.WriteFile(temporaryPath, payload, 0o600); err != nil {
			t.Fatal(err)
		}
		if err := os.Chmod(temporaryPath, 0o444); err != nil {
		...
		store, err := openParticipantOriginatedArtifactStore(root, PublicCaseQuota, false, RoleB)
		if err != nil {
			t.Fatalf("role B failed to recover role A public temporary: %v", err)
		}
		_ = store.close()
		assertNoParticipantOriginatedTemporaryObjects(t, root)
		if published, err := os.ReadFile(filepath.Join(root, submissionAObject)); err != nil || !bytes.Equal(published, payload) {
			t.Fatalf("role A published target was not preserved: %q %v", published, err)
		}
	})

	t.Run("other_role_journal_open_recovers_nonce_temporary", func(t *testing.T) {
		root := filepath.Join(taskTempDir(t, "mordant-participant-cross-role-journal-recovery-"), "journal")
		if err := os.MkdirAll(root, 0o700); err != nil {
			t.Fatal(err)
		}
		nonceA := Digest(sha256.Sum256([]byte("participant-a-nonce")))
		nonceB := Digest(sha256.Sum256([]byte("participant-b-nonce")))
		temporaryPath := filepath.Join(root, ".mordant-create-00000000000000000000000000000003")
		payload := []byte("participant-a-nonce-post-link")
		if err := os.WriteFile(temporaryPath, payload, 0o600); err != nil {
			t.Fatal(err)
		}
		target := participantOriginatedNonceClaimName(nonceA)
		if err := os.Link(temporaryPath, filepath.Join(root, target)); err != nil {
			t.Fatal(err)
		}
		store, err := openParticipantOriginatedJournalStore(root, RoleB, nonceB)
		if err != nil {
			t.Fatalf("role B failed to recover role A nonce temporary: %v", err)
		}
		_ = store.close()
		assertNoParticipantOriginatedTemporaryObjects(t, root)
		if published, err := os.ReadFile(filepath.Join(root, target)); err != nil || !bytes.Equal(published, payload) {
			t.Fatalf("role A nonce target was not preserved: %q %v", published, err)
		}
	})

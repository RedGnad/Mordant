...
// hiding commitment is placed in CipherPledge.PrivateMetadataCommitment.
...
type ParticipantOriginatedImportObject struct {
	Name     string
	Reader   io.Reader
	Expected ObjectRef
}
...
type ParticipantOriginatedStageOptions struct {
	QuarantineRoot string
	Role           string
	Objects        []ParticipantOriginatedImportObject
}

type ParticipantOriginatedStagedArtifact struct {
	Role             string    `json:"role"`
	ArtifactObject   ObjectRef `json:"artifactObject"`
	CiphertextObject ObjectRef `json:"ciphertextObject"`
}

// StageParticipantOriginatedArtifact is optional when the coordinator already
// owns streaming transport. It accepts exactly two declared names, writes them
// through the pinned streaming store, and performs no expensive FHE decoding.
func StageParticipantOriginatedArtifact(options ParticipantOriginatedStageOptions) (ParticipantOriginatedStagedArtifact, error) {
	var staged ParticipantOriginatedStagedArtifact
	_, ciphertextName, manifestName, err := participantFiles(options.Role)
	if err != nil || len(options.Objects) != 2 {
		return staged, ErrParticipantOriginated
	}
	byName := make(map[string]ParticipantOriginatedImportObject, 2)
	for _, object := range options.Objects {
		maximum := participantOriginatedMaximumCiphertextBytes
		if object.Name == manifestName {
			maximum = maxManifestBytes
		}
		if validateObjectName(object.Name) != nil || object.Reader == nil ||
			(object.Name != ciphertextName && object.Name != manifestName) || object.Expected.validate(object.Name, maximum) != nil {
			return staged, ErrParticipantOriginated
		}
		if _, duplicate := byName[object.Name]; duplicate {
			return staged, ErrParticipantOriginated
		}
		byName[object.Name] = object
	}
	if byName[ciphertextName].Reader == nil || byName[manifestName].Reader == nil {
		return staged, ErrParticipantOriginated
	}
	store, err := openParticipantOriginatedArtifactStore(options.QuarantineRoot, PublicCaseQuota, false, options.Role)
	if err != nil {
	...
	if names, _ := store.names(); len(names) != 0 {
		return staged, ErrStore
	}
	// Authenticated transport metadata supplies the exact digest and length for
	// both streams. Ciphertext is staged first; manifest-last is the quarantine
	// commit marker consumed by verification.
	ciphertextExpected := byName[ciphertextName].Expected
	ciphertextRef, err := store.createFromReaderExpected(ciphertextName, byName[ciphertextName].Reader, participantOriginatedMaximumCiphertextBytes, &ciphertextExpected)
	if err != nil {
		return staged, err
	}
	artifactExpected := byName[manifestName].Expected
	artifactRef, err := store.createFromReaderExpected(manifestName, byName[manifestName].Reader, maxManifestBytes, &artifactExpected)
	if err != nil {
		return staged, err
	}
	return ParticipantOriginatedStagedArtifact{Role: options.Role, ArtifactObject: artifactRef, CiphertextObject: ciphertextRef}, nil
}
...
func participantOriginatedArtifactStoreTargets(role string) (map[string]int64, error) {
	_, ciphertextName, manifestName, err := participantFiles(role)
	if err != nil {
		return nil, err
	}
	return map[string]int64{
		ciphertextName: participantOriginatedMaximumCiphertextBytes,
...
func openParticipantOriginatedArtifactStore(root string, quota int64, private bool, role string) (*objectStore, error) {
	targets, err := participantOriginatedArtifactStoreTargets(role)
	if err != nil {
		return nil, err
	}
	return openObjectStoreWithParticipantRecovery(root, quota, private, targets)
}

func participantOriginatedJournalRecoveryTargets(role string, nonce Digest) (map[string]int64, error) {
	admissionName, completionName, err := participantOriginatedImportJournalNames(role)
	if err != nil || !nonzero(nonce) {
		return nil, ErrParticipantOriginated
	}
	return map[string]int64{
		admissionName:  maxManifestBytes,
		completionName: maxManifestBytes,
		participantOriginatedNonceClaimName(nonce): maxManifestBytes,
	}, nil
}

func openParticipantOriginatedJournalStore(root, role string, nonce Digest) (*objectStore, error) {
	targets, err := participantOriginatedJournalRecoveryTargets(role, nonce)
	if err != nil {
		return nil, err
	}
	return openObjectStoreWithParticipantRecovery(root, PrivateCaseQuota, true, targets)
}
...
type ParticipantOriginatedImportOptions struct {
	Stage        ParticipantOriginatedStageOptions
	Verification ParticipantOriginatedVerificationOptions
	JournalRoot  string
	Now          time.Time
}

// ImportParticipantOriginatedArtifact is a convenience composition for CLI
// tests. Coordinators that already stream/authenticate transport should call the
// three explicit Stage / Verify / Publish boundaries instead.
func ImportParticipantOriginatedArtifact(options ParticipantOriginatedImportOptions) (ParticipantOriginatedArtifactVerification, ParticipantOriginatedImportReport, error) {
	if _, err := StageParticipantOriginatedArtifact(options.Stage); err != nil {
		return ParticipantOriginatedArtifactVerification{}, ParticipantOriginatedImportReport{}, err
	}
	verification := options.Verification
	verification.QuarantineRoot = options.Stage.QuarantineRoot
	verified, err := VerifyStagedParticipantOriginatedArtifact(verification)
	if err != nil {
		return verified, ParticipantOriginatedImportReport{}, err
	}
	report, err := PublishParticipantOriginatedArtifact(ParticipantOriginatedPublicationOptions{
		PublicRoot: verification.PublicRoot, QuarantineRoot: verification.QuarantineRoot,
		JournalRoot: options.JournalRoot, Expected: verification.Expected, Now: options.Now,
	})

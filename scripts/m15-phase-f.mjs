#!/usr/bin/env node
/**
 * M-15 phase F. Selects one sub-action of the real engine and stops.
 *
 * Sub-actions: F.commit, F.reveal, F.finalize, F.markDefault, F.releaseA, F.releaseB
 *
...
const STEPS = ['F.commit', 'F.reveal', 'F.finalize', 'F.markDefault', 'F.releaseA', 'F.releaseB'];
const here = dirname(fileURLToPath(import.meta.url));
const argv = process.argv.slice(2);
const stepIndex = argv.indexOf("--step");
const step = stepIndex === -1 ? STEPS[0] : argv[stepIndex + 1];
if (!STEPS.includes(step)) {
  process.stderr.write(`STOP — --step must be one of ${STEPS.join(", ")} for phase F.\n`);
  process.exitCode = 1;
} else {

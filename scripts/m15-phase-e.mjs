#!/usr/bin/env node
/**
 * M-15 phase E. Selects one sub-action of the real engine and stops.
 *
 * Sub-actions: E.sign, E.approve, E.activate
 *
...
const STEPS = ['E.sign', 'E.approve', 'E.activate'];
const here = dirname(fileURLToPath(import.meta.url));
const argv = process.argv.slice(2);
const stepIndex = argv.indexOf("--step");
const step = stepIndex === -1 ? STEPS[0] : argv[stepIndex + 1];
if (!STEPS.includes(step)) {
  process.stderr.write(`STOP — --step must be one of ${STEPS.join(", ")} for phase E.\n`);
  process.exitCode = 1;
} else {
